...
export function MatchPage() {
  ...
  const groupName = isAlphaMode
    ? (alphabets[matchGroup]?.name || matchGroup)
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <SectionHeader title="Study Cards" subtitle={isLargeSet ? 'Review all words' : 'Flip, learn, repeat!'} icon="🎴" darkMode={darkMode} />
        ...
        <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
          {Object.entries(alphabets).map(([key, group]) => (
            <button
              ...
              aria-label={`Study group: ${group.name.split('—')[0].trim()}`}
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap transition-colors focus-visible:ring-2 focus-visible:ring-indigo-400 ${
                matchGroup === key ? 'bg-indigo-500 text-white' : 'bg-slate-800/60 text-slate-300 hover:bg-slate-700'
              }`}
            >
